...
# Preparations
Before we dive into the main task, we need to load the package and an example dataset for our task. The dataset we use is the **pmbc_68k** dataset from [10x Genomics](https://www.10xgenomics.com/resources/datasets/fresh-68-k-pbm-cs-donor-a-1-standard-1-1-0), which consists of >68,000 single cells. With all 32,738 genes.
...
# Task
## With selected PCs of the pilot data, draw the plot and determine sample size using the built-in model

The selected PCs of the pilot data is extract from the dataset imported before:
```{r}
### use pilot data:
p = 15
...
```
In this dataset, *phenoid* is the y label which has 10 classes. In the selected PCs of the pilot data, each class has 15 observations.
...
## With selected PCs of the pilot data, draw the plot and determine sample size using the self-defined model
