...
SSD can determine the sample size for our experiment when we only have a few pilot data. Firstly, it can generate different synthetic training datasets and test datasets. Then it will calculate the corresponding different test classiﬁcation error/ARI/AMI. The final step is to construct the plots in the terms of the sample size and the different metrics (test classiﬁcation error/ARI/AMI). Users can determine the sample size by the above plots. In this supplemenatart materil, we use one real dataset to illustrate our algortithm's performance. The workflow can be seen in Figure 1.

```{r echo=FALSE, fig.cap="Workflow of the SSD package", out.width = '110%'}
knitr::include_graphics("flowchart.jpg")
```

# Preparations
Before we dive into the main task, we need to load the package and an example dataset for our task. The dataset is the **pbmc_68k** dataset from [10x Genomics](https://support.10xgenomics.com/single-cell-gene-expression/datasets).


We pre-processed the dataset: in this dataset, *phenoid* is the response variable which has 10 classes. We sampled 15 observations from the original dataset for each class and assemble them as the pilot data. We normalize and scale the pilot data at first and then run principal component analysis (PCA) for the pilot data and keep 18 PCs according to *JackStrawPlot* and *ElbowPlot* mentioned in [Seurat - Guided Clustering Tutorial](https://satijalab.org/seurat/articles/pbmc3k_tutorial.html). The *JackStrawPlot* and *ElbowPlot* are shown in Figures 2 and 3.
```{r echo=FALSE, fig.cap="JackStrawPlot of the pilot data", out.width = '100%'}
knitr::include_graphics("pbmc_jackstrawplot_pilot.png")
```
```{r echo=FALSE, fig.cap="ElbowPlot of the pilot data", out.width = '80%'}
knitr::include_graphics("pbmc_elbowplot_pilot.png")
```

For the whole dataset, we split the it into training data and test data first, then we ran principal component analysis (PCA). We pre-processed the training dataset using the same stratigies and keep 23 PCs according to *JackStrawPlot* and *ElbowPlot* in Figures 4 and 5,and then project the test set onto the reduced feature space obtained during the training.

```{r echo=FALSE, fig.cap="JackStrawPlot of the true training data", out.width = '110%'}
knitr::include_graphics("pbmc_jackstrawplot.jpeg")
```
```{r echo=FALSE, fig.cap="ElbowPlot of the true training data", out.width = '80%'}
knitr::include_graphics("pbmc_elbowplot.jpeg")
```
...
```
```{r echo=FALSE, fig.cap="The  learning curve of the default setting", out.width = '100%', fig.align = 'center'}
...
```
```{r echo=FALSE, fig.cap="The learning curve of the self-defined function setting", out.width = '100%', fig.align = 'center'}
...
Here, we use the large ture data to compare the sythentic learning curve and true learning curve.
...
```
```{r echo=FALSE, fig.cap="the result plots of comparison using 'ARI' as the index", out.width = '70%', fig.align = 'center'}
...
We could also try when `index` is *classification error* or *AMI* for this dataset. The results are shown in Figure 10-13. The graphs also show that the synthetic plots and the true plots have same trend. 
...
```
```{r echo=FALSE, fig.cap="the result plots of comparison using 'classification error' as the index", out.width = '70%', fig.align = 'center'}
...
result_true_13 = ssd(x=x_true_train, y=y_true_train, model = "randomforest", index="AMI",
                     ...
                     test_x=x_true_test,test_y=y_true_test, n_test=100)
```
```{r echo=FALSE, fig.cap="the result plots of comparison using 'AMI' as the index", out.width = '70%', fig.align = 'center'}
